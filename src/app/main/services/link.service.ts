...
export class LinkService {
  ...
  createLinkItem(categoryId: string, linkItem: LinkItem) {
    const dbRef = this.afdb.database.ref(`${this.authService.authUser.uid}/links/${categoryId}`);

    dbRef.once('value', (links) => {

      const newLinkItem = dbRef.push();
        newLinkItem.set ({
  ...
  moveLink(linkItem: LinkItem, categoryId: string) {
    const oldRef = this.afdb.database.ref(`${this.authService.authUser.uid}/links/${linkItem.categoryId}`).child(linkItem.id);
    const newRef = this.afdb.database.ref(`${this.authService.authUser.uid}/links/${categoryId}`).child(linkItem.id);
  ...
  moveFbRecord(oldRef, newRef) {
    return oldRef.once('value', function(snap)  {
        newRef.set( snap.val(), function(error) {
              if ( !error ) {  oldRef.remove(); } else if ( typeof(console) !== 'undefined' && console.error ) {  console.error(error); }
        });
    });
  }

  updateLinkOrder(linkItem: LinkItem, receivingLink: LinkItem) {
    const linksRef = this.afdb.database.ref(`${this.authService.authUser.uid}/links/${linkItem.categoryId}`);

    const fromIndex = linkItem.order;
    const toIndex = receivingLink.order;
    ...
    linksRef.once('value', (links) => {
      links.forEach((linkSnapShot: any) => {
        const link: LinkItem = linkSnapShot.val();
  ...
  updateLinkItemOrder(linkItem: LinkItem, action: string) {
    const linkRef = this.afdb.database.ref(`${this.authService.authUser.uid}/links/${linkItem.categoryId}`).child(linkItem.id);

    linkRef.once('value', (link) => {
      linkRef.update({
        order: (action === 'add') ? parseInt(link.val().order, 10) + 1 : parseInt(link.val().order, 10) - 1
      }).catch((error) => {
        console.log(error);
      });
    });
